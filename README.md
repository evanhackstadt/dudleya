...
* The Snakefile contains the bulk of the pipeline. It is a list of steps (terminal commands / scripts) to process the data.
* The Snakefile depends on two config files that tell it what to do:
  * profiles/config.yaml — contains parameters telling Snakemake to use SLURM and specifying default resources. We do NOT change this.
  * samples_config/config.yaml — paths to the ref genome, anc genome, and a dictionary mapping sample names to the paths to each raw file (R1 and R2). We DO change this.
* The Snakefile can be run directly, or to run it in the background, we send the submit_snakemake.sh batch file to SLURM. This script just runs the Snakemake command, acting as a wrapper for the overall process.
